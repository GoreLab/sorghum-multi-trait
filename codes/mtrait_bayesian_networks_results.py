...
mlm_dict['MTiLM_cv1'] = pd.read_csv('acc_MTiLM_cv1.csv', header = 0, index_col=0)
mlm_dict['MTiLM_cv2'] = pd.read_csv('acc_MTiLM_cv2.csv', header = 0, index_col=0)
mlm_dict['MTrLM_cv1'] = pd.read_csv('acc_MTrLM_cv1.csv', header = 0, index_col=0)
mlm_dict['MTrLM_cv2'] = pd.read_csv('acc_MTrLM_cv2.csv', header = 0, index_col=0)

# List of models to use:
model_set = ['MTiLM', 'MTrLM']
...
for i in range(len(model_set)):
  ...
  labels_axis1 = ['DAP 60', 'DAP 75', 'DAP 90', 'DAP 105', 'DAP 120']
  # Heat map of the adjusted means across traits:
  heat = sns.heatmap(mlm_dict[model_set[i] + '_cv2'],
             linewidths=0.25,
             vmin=0.3,
             vmax=1,
             annot=True,
